...
A **project** is the unit substrata operates on: a single directory that holds
...
class is the factory that ties these pieces together, and both the command-line
tools and the tutorial notebooks start from it.

This page covers how a project is laid out on disk and the three ways to set one
up:

1. **Initialize it from Metashape** with `substrata metashape-export` — the
   quickest start: point at a Metashape `.psx` and get a complete project folder
   (point cloud, cameras, markers, and a starter YAML) in one command.
2. **Build it in Python, then save a YAML** — load the point cloud, cameras, and
   annotations, compute scale and orientation, and write out a project file you
   can reuse.
3. **Write the YAML by hand** — point at files you already have and describe the
   project declaratively. This section documents the minimum keys, the defaults,
   and the full schema.

Whichever route you take, the end result is the same: a `<project>.yaml` file
that lets you reload the whole project, fully scaled and oriented, with a single
call.
...
└── cur_sna_20m_20200303_anns.csv      # annotation coordinates
```
...
## Avenue 1 — initialize from Metashape with `substrata metashape-export`

The quickest way to start. `substrata metashape-export` initializes a complete
project folder directly from a Metashape `.psx` — the point cloud
(`<id>.ply`), camera calibration/poses (`<id>.cams.xml`, `<id>.meta.json`),
markers (`<id>_markers.csv`), and a starter project file (`<id>.yaml`) — all in
raw chunk-local coordinates. Scale and orientation are left for `substrata
orient` (or the Python workflow below) to compute afterward.

```bash
# Write cur_sna_20m_20200303/ into the current directory, then compute scale + orientation
substrata metashape-export --psx cur_sna_20m_20200303.psx
cd cur_sna_20m_20200303
substrata orient
```

See {doc}`command_line_usage` for the full set of options (choosing a chunk,
output directory, and locating the Metashape executable).

### Exporting the files manually

If you can't run `metashape-export`, you can export the PLY using the Metashape GUI (ensure that it is being exported in local coordinates) and the camera metadata using the following short script:
```
# Export <project>.cams.xml + .meta.json for the active chunk (local coords).
# GUI: Tools > Run Script. Headless: metashape.sh -platform offscreen -r this.py <project.psx>
import json, Metashape

chunk = Metashape.app.document.chunk
base = Metashape.app.document.path.rsplit(".psx", 1)[0]
chunk.exportCameras(base + ".cams.xml")
json.dump({"cameras": {str(c.key): {
    "path": c.photo.path,
    "center": list(c.center) if c.center else None,
    "transform": [list(c.transform.row(n)) for n in range(4)] if c.transform else None,
} for c in chunk.cameras}}, open(base + ".meta.json", "w"), indent=2)
```


## Avenue 2 — build the project in Python, then save the YAML
...
two-liner (see Avenue 3 below).

## Avenue 3 — write the YAML by hand
...
- {doc}`command_line_usage` — drive the same workflows from the terminal.
